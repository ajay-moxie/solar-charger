...
OBJECTS= ADC.p1 Charging.p1 Intensity_Switch.p1 LED.p1 Load_Switch.p1 PWM.p1 SYSTEM.p1 TIMER.p1 temp.p1 interrupt.p1 main.p1 led_load.p1 battery_mgmt.p1
DEPS= ADC.h Charging.h Intensity_Switch.h LED.h Load_Switch.h PWM.h SYSTEM.h TIMER.h temp.h interrupt.h led_load.h pi.h battery_mgmt.h common.h
...
clean:
	rm -f $(OBJECTS) *.hex *.p1 *.pre *.obj *.rlf *.sdb *.hxl *.map *.sym *.cof *.sdb *.dep *.mcs *.lst *.as *.plt
